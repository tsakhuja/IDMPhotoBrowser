...
// Properties
@synthesize displayDoneButton = _displayDoneButton, displayToolbar = _displayToolbar, displayCounterLabel = _displayCounterLabel, useWhiteBackgroundColor = _useWhiteBackgroundColor, doneButtonImage = _doneButtonImage;
...
- (void)viewDidLoad {
    ...
    _toolbarButton = [[UIBarButtonItem alloc] initWithImage:nil
                                                      style:UIBarButtonItemStylePlain
                                                     target:self
                                                     action:@selector(toolbarButtonSelected:)];
    _toolbarButton.tintColor = [UIColor whiteColor];
...
- (void)performLayout {
    ...
    UIBarButtonItem *flexSpace = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace
                                                                               target:self action:nil];
    NSArray *items = @[ flexSpaceLeft, _toolbarButton, flexSpace ];
...
- (CGRect)frameForDoneButtonAtOrientation:(UIInterfaceOrientation)orientation {
    return CGRectMake(5.f, 6.f, 37.f, 30.f);
}
...
- (void)updateToolbar {
    // Counter
    if ([self.delegate respondsToSelector:@selector(photoBrowser:toolBarButtonImageForPhotoAtIndex:)]) {
        _toolbarButton.image = [self.delegate photoBrowser:self toolBarButtonImageForPhotoAtIndex:_currentPageIndex];
    }
...
- (void)toolbarButtonSelected:(UIBarButtonItem *)sender {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:toolBarButtonTappedForPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self toolBarButtonTappedForPhotoAtIndex:_currentPageIndex];
    }
    if ([self.delegate respondsToSelector:@selector(photoBrowser:toolBarButtonImageForPhotoAtIndex:)]) {
        _toolbarButton.image = [self.delegate photoBrowser:self toolBarButtonImageForPhotoAtIndex:_currentPageIndex];
    }
